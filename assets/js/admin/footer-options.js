jQuery(document).ready(function($) {
    ...
    try {
        ...
        // Silent fallback - color picker not available
    }
    ...
    $(document).on('change', 'input[name="ross_theme_footer_options[enable_custom_footer]"]', toggleCustomFooterFields);
    ...
    // Footer template preview (delegated handler)
    ...
    // Apply template (delegated handler)
    $(document).on('click', '#ross-apply-template', function(e) {
        ...
        // disable button while request is ongoing to prevent duplicate requests
        var $btn = $(this);
        ...
        $.post((typeof rossFooterAdmin !== 'undefined' ? rossFooterAdmin.ajax_url : ajaxurl), data, function(resp) {
            ...
            if (resp.success) {
                ...
                var siteLink = (typeof rossFooterAdmin !== 'undefined' && rossFooterAdmin.site_url) ? rossFooterAdmin.site_url : (window.location.origin + '/');
                msg.append('<p>Template applied. <a href="' + widgetsLink + '">Edit widgets</a> — <button class="button-link" id="ross-undo-apply" data-backup-id="' + (resp.data && resp.data.backup_id ? resp.data.backup_id : '') + '">Undo</button> <a class="button-link" href="' + siteLink + '" target="_blank">View site</a></p>');
                    ...
                    $.post((typeof rossFooterAdmin !== 'undefined' ? rossFooterAdmin.ajax_url : ajaxurl), restoreData, function(res) {
                        if (res && res.success) {
                            ...
                            if (res.data && res.data.backups_html) {
                                ...
                                try { reloadFooterBackups(); } catch(e) {}
                            }
                        } else {
    ...
    // Sync templates: open modal with diffs and allow selecting templates to sync
    $(document).on('click', '#ross-sync-templates', function(e) {
        e.preventDefault();
        var $modal = $('#ross-sync-modal');
        $modal.html('<div style="padding:20px;">Loading…</div>').show();
        var data = { action: 'ross_sync_footer_templates', nonce: (typeof rossFooterAdmin !== 'undefined' ? rossFooterAdmin.sync_nonce : '') };
        $.post((typeof rossFooterAdmin !== 'undefined' ? rossFooterAdmin.ajax_url : ajaxurl), data, function(resp) {
            if (resp && resp.success && resp.data && resp.data.html) {
                $modal.html(resp.data.html);
                // Append overlay styling if not present
                if ($('#ross-sync-modal-overlay').length === 0) {
                    $('body').append('<div id="ross-sync-modal-overlay" style="position:fixed;left:0;top:0;right:0;bottom:0;background:rgba(0,0,0,0.4);z-index:9998;display:block;"></div>');
                }
                // Center the modal
                $modal.css({ position:'fixed', zIndex:10000, left:'50%', top:'50%', transform:'translate(-50%,-50%)', background:'#fff', border:'1px solid #ddd', borderRadius:'6px', padding:'16px', maxWidth:'920px', width:'90%', maxHeight:'80vh', overflow:'auto' });
                // Wire apply button inside modal
                $(document).on('click', '#ross-apply-sync', function(ev){
                    ev.preventDefault();
                    var chosen = [];
                    $('#ross-sync-modal input[name="sync_template_ids[]"]:checked').each(function(){ chosen.push($(this).val()); });
                    if (!chosen.length) return alert('Please select at least one template to sync.');
                    if (!confirm('This will overwrite stored templates with the file versions for selected templates. Continue?')) return;
                    var payload = { action: 'ross_apply_template_sync', nonce: (typeof rossFooterAdmin !== 'undefined' ? rossFooterAdmin.sync_nonce : ''), templates: chosen };
                    $.post((typeof rossFooterAdmin !== 'undefined' ? rossFooterAdmin.ajax_url : ajaxurl), payload, function(res){
                        if (res && res.success) {
                            alert('Selected templates synced successfully.');
                            // close modal and reload preview list
                            $('#ross-sync-modal').hide().html('');
                            $('#ross-sync-modal-overlay').remove();
                            try { $('#ross-preview-template').trigger('click'); } catch(e){}
                            // Optionally reload the page to refresh admin UI
                            try { location.reload(); } catch(e){}
                        } else {
                            alert('Sync failed: ' + (res && res.data ? res.data : 'unknown'));
                        }
                    });
                });
                // Wire close
                $(document).on('click', '#ross-sync-close', function(ev){ ev.preventDefault(); $('#ross-sync-modal').hide().html(''); $('#ross-sync-modal-overlay').remove(); });
            } else {
                $modal.html('<div style="padding:16px;">No sync preview available.</div>');
            }
