...
?>>
    ...
    <?php 
    // Check if we should use template content instead of widgets
    $use_template_content = isset($footer_options['use_template_content']) ? intval($footer_options['use_template_content']) : 0;
    
    if ($use_template_content && function_exists('ross_theme_render_template_content')) {
        // Render template content from selected template file
        // Get user's column preference from dropdown
        $columns = isset($footer_options['footer_columns']) ? intval($footer_options['footer_columns']) : 4;
        $columns = max(1, min(4, $columns)); // Validate 1-4
        
        $container_class = (isset($footer_options['footer_width']) && $footer_options['footer_width'] === 'full') ? 'container-fluid' : 'container';
        ?>
        <div class="footer-widgets footer-template-wrapper">
            <div class="<?php echo esc_attr($container_class); ?>">
                <?php ross_theme_render_template_content('', $columns); ?>
            </div>
        </div>
        <?php
    } else {
        // Use traditional WordPress widget areas (always show when not using template content)
        // Always use the user's column setting from the dropdown
        $columns = isset($footer_options['footer_columns']) ? intval($footer_options['footer_columns']) : 4;
        // Validate column count (1-4)
        ...
        <div class="container">
            <div class="copyright-inner" style="text-align:<?php echo esc_attr($copyright_alignment); ?>;">
                ...
                <?php if (ross_theme_should_show_social_icons()): ?>
                    <?php if ( function_exists( 'rosstheme_render_footer_social' ) ) : ?>
                        <?php rosstheme_render_footer_social(); ?>
                    <?php else: ?>
                        <div class="footer-social">
                            <?php if (!empty($footer_options['facebook_url'])): ?>
