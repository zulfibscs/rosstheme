        ...
        <div class="ross-templates-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin: 25px 0;">
            <?php foreach ($templates as $template_id => $template): ?>
                <div class="ross-template-card <?php echo ($current_template === $template_id) ? 'active' : ''; ?>" data-template-id="<?php echo esc_attr($template_id); ?>">
                    ...
                    <!-- Template Preview -->
                    <div class="ross-template-preview" style="background: <?php echo esc_attr(isset($template['bg']) ? $template['bg'] : '#ffffff'); ?>; color: <?php echo esc_attr(isset($template['text']) ? $template['text'] : '#333333'); ?>; padding: 20px; border-radius: 8px 8px 0 0; min-height: 150px; position: relative;">
                        
                        <?php if ($current_template === $template_id): ?>
                            <span class="ross-active-badge" style="position: absolute; top: 10px; right: 10px; background: <?php echo esc_attr(isset($template['accent']) ? $template['accent'] : '#007cba'); ?>; color: #fff; padding: 4px 12px; border-radius: 20px; font-size: 12px; font-weight: 600;">Active</span>
                        <?php endif; ?>
                        
                        <!-- Mini header preview -->
                        <div style="display: flex; justify-content: space-between; align-items: center; padding: 10px 0; border-bottom: 1px solid rgba(<?php echo (isset($template['bg']) && $template['bg'] === '#ffffff') ? '0,0,0' : '255,255,255'; ?>, 0.1);">
                            ...
                            <?php if (isset($template['cta']['enabled']) && $template['cta']['enabled']): ?>
                                <div style="background: <?php echo esc_attr(isset($template['cta']['bg']) ? $template['cta']['bg'] : '#007cba'); ?>; color: <?php echo esc_attr(isset($template['cta']['color']) ? $template['cta']['color'] : '#ffffff'); ?>; padding: 6px 14px; border-radius: <?php echo esc_attr(isset($template['cta']['border_radius']) ? $template['cta']['border_radius'] : '4px'); ?>; font-size: 12px; font-weight: 600;">
                                    <?php echo esc_html(isset($template['cta']['text']) ? $template['cta']['text'] : 'Get Started'); ?>
                                </div>
                            <?php endif; ?>
                        </div>
                        ...
                        <div style="margin-top: 15px; display: flex; gap: 8px; flex-wrap: wrap; font-size: 11px;">
                            <?php if (isset($template['sticky_enabled']) && $template['sticky_enabled']): ?>
                                <span style="background: rgba(<?php echo (isset($template['bg']) && $template['bg'] === '#ffffff') ? '0,0,0' : '255,255,255'; ?>, 0.1); padding: 3px 8px; border-radius: 4px;">📌 Sticky</span>
                            <?php endif; ?>
                            <?php if (isset($template['search_enabled']) && $template['search_enabled']): ?>
                                <span style="background: rgba(<?php echo (isset($template['bg']) && $template['bg'] === '#ffffff') ? '0,0,0' : '255,255,255'; ?>, 0.1); padding: 3px 8px; border-radius: 4px;">🔍 Search</span>
                            <?php endif; ?>
                            <span style="background: rgba(<?php echo (isset($template['bg']) && $template['bg'] === '#ffffff') ? '0,0,0' : '255,255,255'; ?>, 0.1); padding: 3px 8px; border-radius: 4px;">📱 Responsive</span>
                        </div>
                    </div>
                    ...
                    <div class="ross-template-info" style="padding: 15px; background: #fff; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 8px 8px;">
                        <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px;">
                            <span style="font-size: 24px;"><?php echo esc_html(isset($template['icon']) ? $template['icon'] : '📱'); ?></span>
                            <h3 style="margin: 0; font-size: 16px; font-weight: 600;"><?php echo esc_html(isset($template['title']) ? $template['title'] : $template_id); ?></h3>
                        </div>
                        <p style="margin: 0 0 15px; color: #6b7280; font-size: 13px; line-height: 1.5;"><?php echo esc_html(isset($template['description']) ? $template['description'] : ''); ?></p>
