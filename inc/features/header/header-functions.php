 ...
 *
 * Handles header display logic, rendering, and dynamic CSS generation
 * based on theme options. Manages different header styles, sticky behavior,
 * logo display, and topbar functionality.
 *
 * @package RossTheme\Features\Header
 * @since 1.0.0
...
/**
 * Get header options with comprehensive fallbacks
 *
 * Retrieves header theme options and merges with default values
 * to ensure all required options are available with sensible defaults.
 *
 * @return array Header options array with defaults applied
 *
 * @since 1.0.0
 * @uses ross_get_header_options()
 */
function ross_theme_get_header_options() {
    $options = ross_get_header_options();

    // Comprehensive defaults for all header options
    $defaults = array(
...
/**
 * Display the appropriate header based on theme options
 *
 * Determines which header template to load based on user settings.
 * Supports both new template system and legacy header styles.
 * Includes debug output for administrators.
 *
 * @return void
 *
 * @since 1.0.0
...
function ross_theme_display_header() {
    ...
    // Check if using new template system (preferred method)
    if (isset($options['header_template']) && !empty($options['header_template'])) {
        $template_id = $options['header_template'];

        // Map internal template IDs to template part names
        $template_map = array(
            'creative-agency' => 'creative-agency',
            'responsive-pro' => 'responsive-pro'
        );

        if (isset($template_map[$template_id])) {
            // Verify template file exists before loading
    ...
    // Fallback to legacy header_style option for backward compatibility
    ...
    // Load header template using WordPress template hierarchy
...
function ross_theme_header_classes() {
    ...
    if (!empty($options['transparent_homepage']) && is_home() && is_front_page()) {
...
function ross_theme_render_topbar() {
    ...
    $advanced_options = ross_get_advanced_topbar_options();
