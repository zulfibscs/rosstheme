...
class RossHeaderOptions {
    ...
    private function add_responsive_section() {
        add_settings_section(
            'ross_header_responsive_section',
            '📱 Elementor-Style Responsive Controls',
            array($this, 'responsive_section_callback'),
            'ross-theme-header-responsive'
        );
        ...
        add_settings_field(
            'mobile_breakpoint',
            '📱 Mobile Breakpoint (px)',
            array($this, 'responsive_mobile_breakpoint_callback'),
            'ross-theme-header-responsive',
            'ross_header_responsive_section'
        );
        ...
        add_settings_field(
            ...
            'Header Height (px)',
            array($this, 'header_height_responsive_callback'),
            'ross-theme-header-responsive',
            'ross_header_responsive_section'
        );
        ...
        add_settings_field(
            'logo_max_width_responsive',
            'Logo Max Width (px)',
            array($this, 'logo_max_width_responsive_callback'),
            'ross-theme-header-responsive',
            'ross_header_responsive_section'
        );

        // CTA Button Visibility (Responsive)
        add_settings_field(
            'cta_visibility_responsive',
            'Show CTA Button',
            array($this, 'cta_visibility_responsive_callback'),
            'ross-theme-header-responsive',
            'ross_header_responsive_section'
        );
        ...
        add_settings_field(
            'search_visibility_responsive',
            'Show Search',
            array($this, 'search_visibility_responsive_callback'),
            'ross-theme-header-responsive',
            'ross_header_responsive_section'
        );
        ...
        add_settings_field(
            'header_padding_responsive',
            'Header Padding (px)',
            array($this, 'header_padding_responsive_callback'),
            'ross-theme-header-responsive',
            'ross_header_responsive_section'
        );
        ...
        add_settings_field(
            'header_margin_responsive',
            'Header Margin (px)',
            array($this, 'header_margin_responsive_callback'),
            'ross-theme-header-responsive',
            'ross_header_responsive_section'
        );
    }
    ...
    public function responsive_section_callback() {
        echo '<p>Elementor-style responsive controls. Switch between desktop and mobile devices to set different values.</p>';
        echo '<div id="ross-responsive-device-switcher" style="margin: 15px 0; padding: 10px; background: #f8f9fa; border-radius: 4px; border: 1px solid #ddd;">
            <label style="font-weight: 600; margin-right: 15px;">Active Device:</label>
            <button type="button" id="ross-device-desktop" class="ross-device-switcher-btn active" data-device="desktop" style="padding: 8px 16px; margin: 0 5px; border: 2px solid #2271b1; background: #2271b1; color: white; border-radius: 4px; cursor: pointer;">🖥️ Desktop</button>
            <button type="button" id="ross-device-mobile" class="ross-device-switcher-btn" data-device="mobile" style="padding: 8px 16px; margin: 0 5px; border: 2px solid #ddd; background: white; color: #666; border-radius: 4px; cursor: pointer;">📱 Mobile</button>
        </div>';
        echo '<style>
        #ross-theme-header-responsive .form-table { margin-top: 20px; }
        #ross-theme-header-responsive .ross-responsive-input-group { display: flex; align-items: center; gap: 10px; }
        #ross-theme-header-responsive .ross-responsive-input { flex: 1; }
        #ross-theme-header-responsive .ross-device-indicator { font-size: 12px; color: #666; font-weight: 500; }
        .ross-device-switcher-btn.active { background: #2271b1 !important; color: white !important; border-color: #2271b1 !important; }
        .ross-device-switcher-btn:hover { opacity: 0.8; }
        </style>';
    }
    ...
    $sanitized['color_palette'] = isset($input['color_palette']) ? sanitize_text_field($input['color_palette']) : 'professional';
        ...
        $sanitized['desktop_header_enabled'] = isset($input['desktop_header_enabled']) ? 1 : 0;
        $sanitized['desktop_header_height'] = isset($input['desktop_header_height']) ? absint($input['desktop_header_height']) : 80;
        $sanitized['desktop_logo_max_width'] = isset($input['desktop_logo_max_width']) ? absint($input['desktop_logo_max_width']) : 200;
        $sanitized['desktop_menu_alignment'] = isset($input['desktop_menu_alignment']) ? sanitize_text_field($input['desktop_menu_alignment']) : 'center';
        $sanitized['desktop_padding_top'] = isset($input['desktop_padding_top']) ? absint($input['desktop_padding_top']) : 20;
        $sanitized['desktop_padding_right'] = isset($input['desktop_padding_right']) ? absint($input['desktop_padding_right']) : 0;
        $sanitized['desktop_padding_bottom'] = isset($input['desktop_padding_bottom']) ? absint($input['desktop_padding_bottom']) : 20;
        $sanitized['desktop_padding_left'] = isset($input['desktop_padding_left']) ? absint($input['desktop_padding_left']) : 0;
        $sanitized['desktop_cta_visibility'] = isset($input['desktop_cta_visibility']) ? 1 : 0;
        $sanitized['desktop_search_visibility'] = isset($input['desktop_search_visibility']) ? 1 : 0;
        $sanitized['desktop_sticky_behavior'] = isset($input['desktop_sticky_behavior']) ? sanitize_text_field($input['desktop_sticky_behavior']) : 'inherit';
        $sanitized['desktop_font_size'] = isset($input['desktop_font_size']) ? absint($input['desktop_font_size']) : 0;

        // Mobile Header Controls
        $sanitized['mobile_header_enabled'] = isset($input['mobile_header_enabled']) ? 1 : 0;
        $sanitized['mobile_header_height'] = isset($input['mobile_header_height']) ? absint($input['mobile_header_height']) : 60;
        $sanitized['mobile_logo_override'] = isset($input['mobile_logo_override']) ? esc_url_raw($input['mobile_logo_override']) : '';
        $sanitized['mobile_logo_max_width'] = isset($input['mobile_logo_max_width']) ? absint($input['mobile_logo_max_width']) : 120;
        $sanitized['mobile_menu_style'] = isset($input['mobile_menu_style']) ? sanitize_text_field($input['mobile_menu_style']) : 'slide';
        $sanitized['mobile_menu_position'] = isset($input['mobile_menu_position']) ? sanitize_text_field($input['mobile_menu_position']) : 'right';
        $sanitized['mobile_padding_top'] = isset($input['mobile_padding_top']) ? absint($input['mobile_padding_top']) : 15;
        $sanitized['mobile_padding_right'] = isset($input['mobile_padding_right']) ? absint($input['mobile_padding_right']) : 15;
        $sanitized['mobile_padding_bottom'] = isset($input['mobile_padding_bottom']) ? absint($input['mobile_padding_bottom']) : 15;
        $sanitized['mobile_padding_left'] = isset($input['mobile_padding_left']) ? absint($input['mobile_padding_left']) : 15;
        $sanitized['mobile_cta_visibility'] = isset($input['mobile_cta_visibility']) ? 1 : 0;
        $sanitized['mobile_search_visibility'] = isset($input['mobile_search_visibility']) ? 1 : 0;
        $sanitized['mobile_sticky_header'] = isset($input['mobile_sticky_header']) ? sanitize_text_field($input['mobile_sticky_header']) : 'inherit';
        $sanitized['mobile_font_size'] = isset($input['mobile_font_size']) ? absint($input['mobile_font_size']) : 0;

        // Elementor-Style Responsive Settings
        $sanitized['header_height_responsive'] = $this->sanitize_responsive_value($input['header_height_responsive'] ?? array(), array('desktop' => 80, 'mobile' => 60), 'absint');
        $sanitized['logo_max_width_responsive'] = $this->sanitize_responsive_value($input['logo_max_width_responsive'] ?? array(), array('desktop' => 200, 'mobile' => 150), 'absint');
        $sanitized['cta_visibility_responsive'] = $this->sanitize_responsive_value($input['cta_visibility_responsive'] ?? array(), array('desktop' => true, 'mobile' => false), 'bool');
        $sanitized['search_visibility_responsive'] = $this->sanitize_responsive_value($input['search_visibility_responsive'] ?? array(), array('desktop' => true, 'mobile' => true), 'bool');
        $sanitized['header_padding_responsive'] = $this->sanitize_responsive_value($input['header_padding_responsive'] ?? array(), array('desktop' => '20', 'mobile' => '15'), 'text');
        $sanitized['header_margin_responsive'] = $this->sanitize_responsive_value($input['header_margin_responsive'] ?? array(), array('desktop' => '0', 'mobile' => '0'), 'text');

        // Backward Compatibility Migration
        $sanitized = $this->migrate_legacy_responsive_settings($input, $sanitized);
    ...
    /**
     * Sanitize responsive values (desktop/mobile arrays)
     */
    private function sanitize_responsive_value($input, $defaults, $type = 'text') {
        if (!is_array($input)) {
            return $defaults;
        }
        ...
        // Sanitize desktop value
        if (isset($input['desktop'])) {
            $sanitized['desktop'] = $this->sanitize_single_value($input['desktop'], $defaults['desktop'], $type);
        } else {
            $sanitized['desktop'] = $defaults['desktop'];
        }

        // Sanitize mobile value
        if (isset($input['mobile'])) {
            $sanitized['mobile'] = $this->sanitize_single_value($input['mobile'], $defaults['mobile'], $type);
        } else {
            $sanitized['mobile'] = $defaults['mobile'];
        }
     ...
     */
    private function sanitize_single_value($value, $default, $type) {
        switch ($type) {
            case 'absint':
                return absint($value);
            case 'bool':
    ...
    /**
     * Migrate legacy separate desktop/mobile settings to new responsive format
     */
    private function migrate_legacy_responsive_settings($input, $sanitized) {
        // Only migrate if responsive settings are empty (first time setup)
        if (empty($input['header_height_responsive']['desktop']) && empty($input['header_height_responsive']['mobile'])) {
            // Header Height Migration
            $desktop_height = isset($input['desktop_header_height']) ? absint($input['desktop_header_height']) : (isset($input['header_height']) ? absint($input['header_height']) : 80);
            $mobile_height = isset($input['mobile_header_height']) ? absint($input['mobile_header_height']) : 60;
            $sanitized['header_height_responsive'] = array('desktop' => $desktop_height, 'mobile' => $mobile_height);

            // Logo Width Migration
            $desktop_logo = isset($input['desktop_logo_max_width']) ? absint($input['desktop_logo_max_width']) : (isset($input['logo_width']) ? absint($input['logo_width']) : 200);
            $mobile_logo = isset($input['mobile_logo_max_width']) ? absint($input['mobile_logo_max_width']) : 150;
            $sanitized['logo_max_width_responsive'] = array('desktop' => $desktop_logo, 'mobile' => $mobile_logo);

            // CTA Visibility Migration
            $desktop_cta = isset($input['desktop_cta_visibility']) ? (bool)$input['desktop_cta_visibility'] : (isset($input['enable_cta_button']) ? (bool)$input['enable_cta_button'] : true);
            $mobile_cta = isset($input['mobile_cta_visibility']) ? (bool)$input['mobile_cta_visibility'] : false;
            $sanitized['cta_visibility_responsive'] = array('desktop' => $desktop_cta, 'mobile' => $mobile_cta);

            // Search Visibility Migration
            $desktop_search = isset($input['desktop_search_visibility']) ? (bool)$input['desktop_search_visibility'] : (isset($input['enable_search']) ? (bool)$input['enable_search'] : true);
            $mobile_search = isset($input['mobile_search_visibility']) ? (bool)$input['mobile_search_visibility'] : true;
            $sanitized['search_visibility_responsive'] = array('desktop' => $desktop_search, 'mobile' => $mobile_search);

            // Header Padding Migration
            $desktop_padding = isset($input['desktop_padding_top']) ? $input['desktop_padding_top'] : '20';
            $mobile_padding = isset($input['mobile_padding_top']) ? $input['mobile_padding_top'] : '15';
            $sanitized['header_padding_responsive'] = array('desktop' => $desktop_padding, 'mobile' => $mobile_padding);

            // Header Margin Migration
            $desktop_margin = isset($input['desktop_margin_top']) ? $input['desktop_margin_top'] : '0';
            $mobile_margin = isset($input['mobile_margin_top']) ? $input['mobile_margin_top'] : '0';
            $sanitized['header_margin_responsive'] = array('desktop' => $desktop_margin, 'mobile' => $mobile_margin);
        }

        return $sanitized;
    }

    // Responsive Header Field Callbacks

    public function desktop_header_enabled_callback() {
        $value = isset($this->options['desktop_header_enabled']) ? $this->options['desktop_header_enabled'] : 1;
        ?>
        <input type="checkbox" name="ross_theme_header_options[desktop_header_enabled]" value="1" <?php checked(1, $value); ?> />
        <label>Enable desktop header layout</label>
        <p class="description">Show desktop header above the mobile breakpoint</p>
        <?php
    }

    public function desktop_header_height_callback() {
        $value = isset($this->options['desktop_header_height']) ? absint($this->options['desktop_header_height']) : 80;
        ?>
        <input type="number" name="ross_theme_header_options[desktop_header_height]" value="<?php echo esc_attr($value); ?>" class="small-text" min="40" max="200" /> px
        <p class="description">Header height on desktop screens</p>
        <?php
    }

    public function desktop_logo_max_width_callback() {
        $value = isset($this->options['desktop_logo_max_width']) ? absint($this->options['desktop_logo_max_width']) : 200;
        ?>
        <input type="number" name="ross_theme_header_options[desktop_logo_max_width]" value="<?php echo esc_attr($value); ?>" class="small-text" min="50" max="500" /> px
        <p class="description">Maximum logo width on desktop</p>
        <?php
    }

    public function desktop_menu_alignment_callback() {
        $value = isset($this->options['desktop_menu_alignment']) ? $this->options['desktop_menu_alignment'] : 'center';
        ?>
        <select name="ross_theme_header_options[desktop_menu_alignment]">
            <option value="left" <?php selected($value, 'left'); ?>>Left</option>
            <option value="center" <?php selected($value, 'center'); ?>>Center</option>
            <option value="right" <?php selected($value, 'right'); ?>>Right</option>
        </select>
        <p class="description">Navigation menu alignment on desktop</p>
        <?php
    }

    public function desktop_padding_callback() {
        $top = isset($this->options['desktop_padding_top']) ? absint($this->options['desktop_padding_top']) : 20;
        $right = isset($this->options['desktop_padding_right']) ? absint($this->options['desktop_padding_right']) : 0;
        $bottom = isset($this->options['desktop_padding_bottom']) ? absint($this->options['desktop_padding_bottom']) : 20;
        $left = isset($this->options['desktop_padding_left']) ? absint($this->options['desktop_padding_left']) : 0;
        ?>
        <div style="display: flex; gap: 10px; align-items: center;">
            <input type="number" name="ross_theme_header_options[desktop_padding_top]" value="<?php echo esc_attr($top); ?>" class="small-text" placeholder="Top" min="0" max="100" />
            <input type="number" name="ross_theme_header_options[desktop_padding_right]" value="<?php echo esc_attr($right); ?>" class="small-text" placeholder="Right" min="0" max="100" />
            <input type="number" name="ross_theme_header_options[desktop_padding_bottom]" value="<?php echo esc_attr($bottom); ?>" class="small-text" placeholder="Bottom" min="0" max="100" />
            <input type="number" name="ross_theme_header_options[desktop_padding_left]" value="<?php echo esc_attr($left); ?>" class="small-text" placeholder="Left" min="0" max="100" />
            <span>px</span>
        </div>
        <p class="description">Top, Right, Bottom, Left padding for desktop header</p>
        <?php
    }

    public function desktop_cta_visibility_callback() {
        $value = isset($this->options['desktop_cta_visibility']) ? $this->options['desktop_cta_visibility'] : 1;
        ?>
        <input type="checkbox" name="ross_theme_header_options[desktop_cta_visibility]" value="1" <?php checked(1, $value); ?> />
        <label>Show CTA button on desktop</label>
        <p class="description">Display call-to-action button in desktop header</p>
        <?php
    }

    public function desktop_search_visibility_callback() {
        $value = isset($this->options['desktop_search_visibility']) ? $this->options['desktop_search_visibility'] : 1;
        ?>
        <input type="checkbox" name="ross_theme_header_options[desktop_search_visibility]" value="1" <?php checked(1, $value); ?> />
        <label>Show search on desktop</label>
        <p class="description">Display search functionality in desktop header</p>
        <?php
    }

    public function desktop_sticky_behavior_callback() {
        $value = isset($this->options['desktop_sticky_behavior']) ? $this->options['desktop_sticky_behavior'] : 'inherit';
        ?>
        <select name="ross_theme_header_options[desktop_sticky_behavior]">
            <option value="inherit" <?php selected($value, 'inherit'); ?>>Inherit from main settings</option>
            <option value="always" <?php selected($value, 'always'); ?>>Always sticky</option>
            <option value="never" <?php selected($value, 'never'); ?>>Never sticky</option>
            <option value="after_threshold" <?php selected($value, 'after_threshold'); ?>>Sticky after scroll threshold</option>
        </select>
        <p class="description">Override sticky behavior for desktop screens</p>
        <?php
    }

    public function desktop_font_size_callback() {
        $value = isset($this->options['desktop_font_size']) ? absint($this->options['desktop_font_size']) : 0;
        ?>
        <input type="number" name="ross_theme_header_options[desktop_font_size]" value="<?php echo esc_attr($value); ?>" class="small-text" min="0" max="24" /> px
        <p class="description">Override menu font size on desktop (0 = use main setting)</p>
        <?php
    }

    public function responsive_mobile_breakpoint_callback() {
        $value = isset($this->options['mobile_breakpoint']) ? absint($this->options['mobile_breakpoint']) : 768;
        ?>
        <input type="number" name="ross_theme_header_options[mobile_breakpoint]" value="<?php echo esc_attr($value); ?>" class="small-text" min="320" max="1200" /> px
        <p class="description">Screen width where mobile layout takes over</p>
        <?php
    }

    public function desktop_section_header_callback() {
        echo '<strong>Desktop Header Settings</strong><br><small>Controls for screens above the mobile breakpoint</small>';
    }

    public function mobile_section_header_callback() {
        echo '<strong>Mobile Header Settings</strong><br><small>Controls for screens below the mobile breakpoint</small>';
    }

    public function mobile_header_enabled_callback() {
        $value = isset($this->options['mobile_header_enabled']) ? $this->options['mobile_header_enabled'] : 1;
        ?>
        <input type="checkbox" name="ross_theme_header_options[mobile_header_enabled]" value="1" <?php checked(1, $value); ?> />
        <label>Enable mobile header layout</label>
        <p class="description">Show mobile header below the breakpoint</p>
        <?php
    }

    public function mobile_header_height_callback() {
        $value = isset($this->options['mobile_header_height']) ? absint($this->options['mobile_header_height']) : 60;
        ?>
        <input type="number" name="ross_theme_header_options[mobile_header_height]" value="<?php echo esc_attr($value); ?>" class="small-text" min="40" max="120" /> px
        <p class="description">Header height on mobile screens</p>
        <?php
    }

    public function mobile_logo_override_callback() {
        $value = isset($this->options['mobile_logo_override']) ? $this->options['mobile_logo_override'] : '';
        ?>
        <input type="text" name="ross_theme_header_options[mobile_logo_override]" id="mobile_logo_override" value="<?php echo esc_url($value); ?>" class="regular-text" />
        <input type="button" class="button ross-upload-button" data-target="mobile_logo_override" value="Upload Mobile Logo" />
        <?php if ($value): ?>
            <p><img src="<?php echo esc_url($value); ?>" style="max-width: 120px; height: auto; margin-top: 10px;" /></p>
        <?php endif; ?>
        <p class="description">Optional separate logo for mobile (leave empty to use main logo)</p>
        <?php
    }

    public function mobile_logo_max_width_callback() {
        $value = isset($this->options['mobile_logo_max_width']) ? absint($this->options['mobile_logo_max_width']) : 120;
        ?>
        <input type="number" name="ross_theme_header_options[mobile_logo_max_width]" value="<?php echo esc_attr($value); ?>" class="small-text" min="50" max="300" /> px
        <p class="description">Maximum logo width on mobile</p>
        <?php
    }

    public function responsive_mobile_menu_style_callback() {
        $value = isset($this->options['mobile_menu_style']) ? $this->options['mobile_menu_style'] : 'slide';
        ?>
        <select name="ross_theme_header_options[mobile_menu_style]">
            <option value="slide" <?php selected($value, 'slide'); ?>>Slide Down</option>
            <option value="dropdown" <?php selected($value, 'dropdown'); ?>>Dropdown</option>
            <option value="fullscreen" <?php selected($value, 'fullscreen'); ?>>Fullscreen Overlay</option>
        </select>
        <p class="description">How the mobile menu appears when opened</p>
        <?php
    }

    public function responsive_mobile_menu_position_callback() {
        $value = isset($this->options['mobile_menu_position']) ? $this->options['mobile_menu_position'] : 'right';
        ?>
        <select name="ross_theme_header_options[mobile_menu_position]">
            <option value="left" <?php selected($value, 'left'); ?>>Left Side</option>
            <option value="right" <?php selected($value, 'right'); ?>>Right Side</option>
        </select>
        <p class="description">Position of mobile menu toggle button</p>
        <?php
    }

    public function mobile_padding_callback() {
        $top = isset($this->options['mobile_padding_top']) ? absint($this->options['mobile_padding_top']) : 15;
        $right = isset($this->options['mobile_padding_right']) ? absint($this->options['mobile_padding_right']) : 15;
        $bottom = isset($this->options['mobile_padding_bottom']) ? absint($this->options['mobile_padding_bottom']) : 15;
        $left = isset($this->options['mobile_padding_left']) ? absint($this->options['mobile_padding_left']) : 15;
        ?>
        <div style="display: flex; gap: 10px; align-items: center;">
            <input type="number" name="ross_theme_header_options[mobile_padding_top]" value="<?php echo esc_attr($top); ?>" class="small-text" placeholder="Top" min="0" max="50" />
            <input type="number" name="ross_theme_header_options[mobile_padding_right]" value="<?php echo esc_attr($right); ?>" class="small-text" placeholder="Right" min="0" max="50" />
            <input type="number" name="ross_theme_header_options[mobile_padding_bottom]" value="<?php echo esc_attr($bottom); ?>" class="small-text" placeholder="Bottom" min="0" max="50" />
            <input type="number" name="ross_theme_header_options[mobile_padding_left]" value="<?php echo esc_attr($left); ?>" class="small-text" placeholder="Left" min="0" max="50" />
            <span>px</span>
        </div>
        <p class="description">Top, Right, Bottom, Left padding for mobile header</p>
        <?php
    }

    public function mobile_cta_visibility_callback() {
        $value = isset($this->options['mobile_cta_visibility']) ? $this->options['mobile_cta_visibility'] : 1;
        ?>
        <input type="checkbox" name="ross_theme_header_options[mobile_cta_visibility]" value="1" <?php checked(1, $value); ?> />
        <label>Show CTA button on mobile</label>
        <p class="description">Display call-to-action button in mobile menu</p>
        <?php
    }

    public function mobile_search_visibility_callback() {
        $value = isset($this->options['mobile_search_visibility']) ? $this->options['mobile_search_visibility'] : 1;
        ?>
        <input type="checkbox" name="ross_theme_header_options[mobile_search_visibility]" value="1" <?php checked(1, $value); ?> />
        <label>Show search on mobile</label>
        <p class="description">Display search functionality in mobile header</p>
        <?php
    }

    public function responsive_mobile_sticky_header_callback() {
        $value = isset($this->options['mobile_sticky_header']) ? $this->options['mobile_sticky_header'] : 'inherit';
        ?>
        <select name="ross_theme_header_options[mobile_sticky_header]">
            <option value="inherit" <?php selected($value, 'inherit'); ?>>Inherit from main settings</option>
            <option value="always" <?php selected($value, 'always'); ?>>Always sticky</option>
            <option value="never" <?php selected($value, 'never'); ?>>Never sticky</option>
        </select>
        <p class="description">Override sticky behavior for mobile screens</p>
        <?php
    }

    public function mobile_font_size_callback() {
        $value = isset($this->options['mobile_font_size']) ? absint($this->options['mobile_font_size']) : 0;
        ?>
        <input type="number" name="ross_theme_header_options[mobile_font_size]" value="<?php echo esc_attr($value); ?>" class="small-text" min="0" max="20" /> px
        <p class="description">Override menu font size on mobile (0 = use main setting)</p>
        <?php
    }

    // Elementor-Style Responsive Callbacks
    public function header_height_responsive_callback() {
        $value = isset($this->options['header_height_responsive']) ? $this->options['header_height_responsive'] : array('desktop' => 80, 'mobile' => 60);
        if (!is_array($value)) {
            $value = array('desktop' => 80, 'mobile' => 60);
        }
        $desktop_value = isset($value['desktop']) ? absint($value['desktop']) : 80;
        $mobile_value = isset($value['mobile']) ? absint($value['mobile']) : 60;
        ?>
        <div class="ross-responsive-input-group">
            <input type="number" name="ross_theme_header_options[header_height_responsive][desktop]" value="<?php echo esc_attr($desktop_value); ?>" class="small-text ross-responsive-input ross-device-desktop" min="40" max="200" data-device="desktop" />
            <input type="number" name="ross_theme_header_options[header_height_responsive][mobile]" value="<?php echo esc_attr($mobile_value); ?>" class="small-text ross-responsive-input ross-device-mobile" min="40" max="120" data-device="mobile" style="display:none;" />
            <span class="ross-device-indicator">px</span>
        </div>
        <p class="description">Header height for desktop and mobile devices</p>
        <?php
    }

    public function logo_max_width_responsive_callback() {
        $value = isset($this->options['logo_max_width_responsive']) ? $this->options['logo_max_width_responsive'] : array('desktop' => 200, 'mobile' => 150);
        if (!is_array($value)) {
            $value = array('desktop' => 200, 'mobile' => 150);
        }
        $desktop_value = isset($value['desktop']) ? absint($value['desktop']) : 200;
        $mobile_value = isset($value['mobile']) ? absint($value['mobile']) : 150;
        ?>
        <div class="ross-responsive-input-group">
            <input type="number" name="ross_theme_header_options[logo_max_width_responsive][desktop]" value="<?php echo esc_attr($desktop_value); ?>" class="small-text ross-responsive-input ross-device-desktop" min="50" max="500" data-device="desktop" />
            <input type="number" name="ross_theme_header_options[logo_max_width_responsive][mobile]" value="<?php echo esc_attr($mobile_value); ?>" class="small-text ross-responsive-input ross-device-mobile" min="50" max="300" data-device="mobile" style="display:none;" />
            <span class="ross-device-indicator">px</span>
        </div>
        <p class="description">Maximum logo width for desktop and mobile devices</p>
        <?php
    }

    public function cta_visibility_responsive_callback() {
        $value = isset($this->options['cta_visibility_responsive']) ? $this->options['cta_visibility_responsive'] : array('desktop' => true, 'mobile' => false);
        if (!is_array($value)) {
            $value = array('desktop' => true, 'mobile' => false);
        }
        $desktop_value = isset($value['desktop']) ? (bool)$value['desktop'] : true;
        $mobile_value = isset($value['mobile']) ? (bool)$value['mobile'] : false;
        ?>
        <div class="ross-responsive-input-group">
            <input type="checkbox" name="ross_theme_header_options[cta_visibility_responsive][desktop]" value="1" <?php checked(1, $desktop_value); ?> class="ross-responsive-input ross-device-desktop" data-device="desktop" />
            <label class="ross-device-indicator ross-device-desktop" data-device="desktop">Desktop</label>
            <input type="checkbox" name="ross_theme_header_options[cta_visibility_responsive][mobile]" value="1" <?php checked(1, $mobile_value); ?> class="ross-responsive-input ross-device-mobile" data-device="mobile" style="display:none;" />
            <label class="ross-device-indicator ross-device-mobile" data-device="mobile" style="display:none;">Mobile</label>
        </div>
        <p class="description">Show CTA button on desktop and mobile devices</p>
        <?php
    }

    public function search_visibility_responsive_callback() {
        $value = isset($this->options['search_visibility_responsive']) ? $this->options['search_visibility_responsive'] : array('desktop' => true, 'mobile' => true);
        if (!is_array($value)) {
            $value = array('desktop' => true, 'mobile' => true);
        }
        $desktop_value = isset($value['desktop']) ? (bool)$value['desktop'] : true;
        $mobile_value = isset($value['mobile']) ? (bool)$value['mobile'] : true;
        ?>
        <div class="ross-responsive-input-group">
            <input type="checkbox" name="ross_theme_header_options[search_visibility_responsive][desktop]" value="1" <?php checked(1, $desktop_value); ?> class="ross-responsive-input ross-device-desktop" data-device="desktop" />
            <label class="ross-device-indicator ross-device-desktop" data-device="desktop">Desktop</label>
            <input type="checkbox" name="ross_theme_header_options[search_visibility_responsive][mobile]" value="1" <?php checked(1, $mobile_value); ?> class="ross-responsive-input ross-device-mobile" data-device="mobile" style="display:none;" />
            <label class="ross-device-indicator ross-device-mobile" data-device="mobile" style="display:none;">Mobile</label>
        </div>
        <p class="description">Show search functionality on desktop and mobile devices</p>
        <?php
    }

    public function header_padding_responsive_callback() {
        $value = isset($this->options['header_padding_responsive']) ? $this->options['header_padding_responsive'] : array('desktop' => '20', 'mobile' => '15');
        if (!is_array($value)) {
            $value = array('desktop' => '20', 'mobile' => '15');
        }
        $desktop_value = isset($value['desktop']) ? sanitize_text_field($value['desktop']) : '20';
        $mobile_value = isset($value['mobile']) ? sanitize_text_field($value['mobile']) : '15';
        ?>
        <div class="ross-responsive-input-group">
            <input type="text" name="ross_theme_header_options[header_padding_responsive][desktop]" value="<?php echo esc_attr($desktop_value); ?>" class="small-text ross-responsive-input ross-device-desktop" placeholder="20 20 20 20" data-device="desktop" />
            <input type="text" name="ross_theme_header_options[header_padding_responsive][mobile]" value="<?php echo esc_attr($mobile_value); ?>" class="small-text ross-responsive-input ross-device-mobile" placeholder="15 15 15 15" data-device="mobile" style="display:none;" />
            <span class="ross-device-indicator">px (top right bottom left)</span>
        </div>
        <p class="description">Header padding for desktop and mobile devices (CSS shorthand)</p>
        <?php
    }

    public function header_margin_responsive_callback() {
        $value = isset($this->options['header_margin_responsive']) ? $this->options['header_margin_responsive'] : array('desktop' => '0', 'mobile' => '0');
        if (!is_array($value)) {
            $value = array('desktop' => '0', 'mobile' => '0');
        }
        $desktop_value = isset($value['desktop']) ? sanitize_text_field($value['desktop']) : '0';
        $mobile_value = isset($value['mobile']) ? sanitize_text_field($value['mobile']) : '0';
        ?>
        <div class="ross-responsive-input-group">
            <input type="text" name="ross_theme_header_options[header_margin_responsive][desktop]" value="<?php echo esc_attr($desktop_value); ?>" class="small-text ross-responsive-input ross-device-desktop" placeholder="0 0 0 0" data-device="desktop" />
            <input type="text" name="ross_theme_header_options[header_margin_responsive][mobile]" value="<?php echo esc_attr($mobile_value); ?>" class="small-text ross-responsive-input ross-device-mobile" placeholder="0 0 0 0" data-device="mobile" style="display:none;" />
            <span class="ross-device-indicator">px (top right bottom left)</span>
        </div>
        <p class="description">Header margin for desktop and mobile devices (CSS shorthand)</p>
        <?php
    }
